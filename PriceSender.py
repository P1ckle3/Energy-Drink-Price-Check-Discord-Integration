...
def wooliesCheck(hooklink):
    ...
    def get_fresh_cookies():
        print("Getting fresh cookies from Woolworths...")
        hook.send("Getting fresh cookies from Woolworths...")
        options = Options()
        ...
        print("Waiting for cookies to be set (15 sec)")
        hook.send("Waiting for cookies to be set (15 sec)")
        time.sleep(15)  # Wait for cookies to be set
        cookies = driver.get_cookies()
        driver.quit()
        if not cookies:
            hook.send("No cookies retrieved from Woolworths!")
    ...
    hook.send("Itterating through products...")
    for p in products:
    ...
    hook.send("Generatig embed for Discord...")
    embed = Embed(
